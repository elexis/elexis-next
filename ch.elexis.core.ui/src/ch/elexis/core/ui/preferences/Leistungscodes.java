...
import ch.elexis.core.ui.controls.ListDisplay;
...
public class Leistungscodes extends PreferencePage implements IWorkbenchPreferencePage {
	...
	String[] systeme = Hub.globalCfg.nodes(PreferenceConstants.LEISTUNGSCODES_CFG_KEY);
	Table table;
	String[] tableCols = {
	...
	protected Control createContents(final Composite parent){
		...
		SWTHelper.createHyperlink(ret, Messages.Leistungscodes_new, new HyperlinkAdapter() {
			...
			public void linkActivated(final HyperlinkEvent e){
				...
				if (at.open() == Dialog.OK) {
					...
					String key = PreferenceConstants.LEISTUNGSCODES_CFG_KEY + "/" + result[0]; //$NON-NLS-1$
					...
					Hub.globalCfg.set(key + "/fakultativ", result[4]); //$NON-NLS-1$
					Hub.globalCfg.set(key + "/unused", result[5]); //$NON-NLS-1$
					Hub.globalCfg.set(key + "/disabled", result[6]); //$NON-NLS-1$
					systeme = Hub.globalCfg.nodes(PreferenceConstants.LEISTUNGSCODES_CFG_KEY);
					reload();
				}
			}
		});
		...
		// *** checkboxes strict check and optify check
		...
		bStrictCheck.addSelectionListener(new SelectionAdapter() {
			...
			public void widgetSelected(final SelectionEvent e){
				Hub.userCfg.set(PreferenceConstants.LEISTUNGSCODES_BILLING_STRICT, bStrictCheck.getSelection());
			}
		});
		bStrictCheck.setSelection(Hub.userCfg.get(PreferenceConstants.LEISTUNGSCODES_BILLING_STRICT, true));
		...
		bOptify.addSelectionListener(new SelectionAdapter() {
			@Override
			public void widgetSelected(final SelectionEvent e){
				Hub.userCfg.set(PreferenceConstants.LEISTUNGSCODES_OPTIFY, bOptify.getSelection());
			}
			
		});
		bOptify.setSelection(Hub.userCfg.get(PreferenceConstants.LEISTUNGSCODES_OPTIFY, true));
		...
		bObligation.addSelectionListener(new SelectionAdapter() {
			@Override
			public void widgetSelected(final SelectionEvent e){
				Hub.userCfg.set(PreferenceConstants.LEISTUNGSCODES_OBLIGATION, bObligation.getSelection());
			}
			
		});
		bObligation.setSelection(Hub.userCfg.get(PreferenceConstants.LEISTUNGSCODES_OBLIGATION, false));
	...
	public void reload(){
		...
		if (systeme != null) {
			for (String s : systeme) {
				String cfgkey = PreferenceConstants.LEISTUNGSCODES_CFG_KEY + "/" + s + "/"; //$NON-NLS-1$ //$NON-NLS-2$
				TableItem it = new TableItem(table, SWT.NONE);
				String name = Hub.globalCfg.get(cfgkey + "name", "default"); //$NON-NLS-1$ //$NON-NLS-2$
				it.setText(0, name);
				it.setText(1, Hub.globalCfg.get(cfgkey + "leistungscodes", "?")); //$NON-NLS-1$ //$NON-NLS-2$
	...
	class AbrechnungsTypDialog_InputDialog extends Dialog {
		...
		protected Control createDialogArea(final Composite parent){
			...
			if (cHasStyledCheckbox & (Hub.actUser.getLabel().equalsIgnoreCase("a"))) { //$NON-NLS-1$
	...
	class AbrechnungsTypDialog extends TitleAreaDialog {
		...
		protected Control createDialogArea(final Composite parent){
			...
			if (Hub.acl.request(AccessControlDefaults.CASE_DEFINE_SPECIALS) == true) {
				data = null;
				...
				fdUnused.addMoveToAction(ldRequirements,
					Messages.Leistungscodes_moveItemToRequiredData, Images.IMG_MOVETOUPPERLIST.name(), true);
				fdUnused.addMoveToAction(ldOptional,
					Messages.Leistungscodes_moveItemToOptionalData, Images.IMG_MOVETOLOWERLIST.name(), true);
	...
	class FieldDefsDisplay extends Composite {
		...
		public void addMoveToAction(ListDisplay<String> destinationList, String toolTipText,
			...
			if (toolBar != null) {
				...
				addToolItem(toolBar, image, toolTipText, newItemIx, enabled);
				ListPopUpMenuAction newAction =
					new ListPopUpMenuAction(toolTipText, image, toolTipText, newItemIx,
		...
		public FieldDefsDisplay(Composite parent, int style, String[] listItems){
			...
			addToolItem(toolBar, Images.IMG_ARROWUP.name(), Messages.Leistungscodes_moveItemUp, MOVEITEMUP,
				true);
			addToolItem(toolBar, Images.IMG_ARROWDOWN.name(), Messages.Leistungscodes_moveItemDown,
				MOVEITEMDOWN, true);
			addToolItem(toolBar, null, Messages.Leistungscodes_moveItemDown, -1, false);
			addToolItem(toolBar, Images.IMG_REMOVEITEM.name(), Messages.Leistungscodes_deleteItem,
				DELETEITEM, true);
			addToolItem(toolBar, Images.IMG_EDIT.name(), Messages.Leistungscodes_editItem, EDITITEM, true);
			addToolItem(toolBar, null, Messages.Leistungscodes_moveItemDown, -1, false);
			
			// ****** create popupmenu for this list (same actions)
			ListPopUpMenuAction moveItemUpAction =
				new ListPopUpMenuAction(Messages.Leistungscodes_moveItemUp, Images.IMG_ARROWUP.name(),
					Messages.Leistungscodes_moveItemUp, MOVEITEMUP, listDisplay, true);
			ListPopUpMenuAction moveItemDownAction =
				new ListPopUpMenuAction(Messages.Leistungscodes_moveItemDown, Images.IMG_ARROWDOWN.name(),
					Messages.Leistungscodes_moveItemDown, MOVEITEMDOWN, listDisplay, true);
			ListPopUpMenuAction delItemAction =
				new ListPopUpMenuAction(Messages.Leistungscodes_deleteAction, Images.IMG_REMOVEITEM.name(),
					Messages.Leistungscodes_removeConstraintTT, DELETEITEM, listDisplay, true);
			ListPopUpMenuAction changeItemAction =
				new ListPopUpMenuAction(Messages.Leistungscodes_editItem, Images.IMG_EDIT.name(),
		 ...
		 *            enabled or disabled...
		 */
		private void addToolItem(ToolBar parent, String image, String toolTipText, int actionType,
			boolean enabled){
			if (actionType == -1) {
				ToolItem toolItem = new ToolItem(parent, SWT.SEPARATOR);
				toolItem.setEnabled(false);
			} else {
				ImageRegistry imageRegistry = Desk.getImageRegistry();
				ToolItem toolItem = new ToolItem(parent, SWT.PUSH);
				toolItem.setToolTipText(toolTipText);
				toolItem.setImage(imageRegistry.get(image));
		...
		class ListPopUpMenuAction extends Action {
			...
			public ListPopUpMenuAction(String actionName, String image, String toolTipText,
				int actionType, ListDisplay<String> listDisplay, boolean enabled){
				super(actionName);
				this.actionType = actionType;
				setImageDescriptor(Desk.getImageDescriptor(image));
