...
public class KontaktErfassenDialog extends TitleAreaDialog {
	...
	protected Control createDialogArea(final Composite parent){
		...
		Composite cTypes = Activator.getToolkit().createComposite(typeComp, SWT.BORDER);
		bOrganisation =
				Activator.getToolkit().createButton(cTypes,
		...
		hlAnschrift =
				Activator.getToolkit().createHyperlink(ret,
					...
					if (bOrganisation.getSelection() && k.istOrganisation()) {
						...
						if (MessageDialog
							.openConfirm(getShell(), Messages
								...
								Messages
									.getString("KontaktErfassenDialog.organisationExistiert.msg")) == false) { //$NON-NLS-1$
							super.okPressed();
							return;
						}
					}
