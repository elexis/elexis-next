...
public class VerrechnungsDisplay extends Composite {
	...
	public VerrechnungsDisplay(final IWorkbenchPage page, Composite parent, int style){
		...
		hVer =
				Activator.getToolkit().createHyperlink(this,
		...
		makeActions();
		tVerr = Activator.getToolkit().createTable(this, SWT.SINGLE);
		...
		ElexisEventDispatcher.getInstance().addListeners(
			new ElexisEventListenerImpl(Verrechnet.class, ElexisEvent.EVENT_UPDATE) {
				@Override
	...
	private void makeActions(){
		...
		chPriceAction = new Action(CHPRICE) {
			...
			public void run(){
				...
				InputDialog dlg =
					new InputDialog(Desk.getTopShell(),
