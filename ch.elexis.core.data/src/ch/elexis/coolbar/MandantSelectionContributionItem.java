...
public class MandantSelectionContributionItem extends ContributionItem {
	...
	public void fill(ToolBar parent, int index){
		...
		for (int i = 0; i < mandants.length; i++) {
			...
			if (Hub.actMandant != null && Hub.actMandant.equals(m)) {
				menuItems[i].setSelection(true);
			} else {
				menuItems[i].setSelection(false);
			}
		...
		if (Hub.actMandant != null && item != null) {
			item.setText(Hub.actMandant.getMandantLabel());
			fParent.setBackground(getColorForMandator(Hub.actMandant));
		}
